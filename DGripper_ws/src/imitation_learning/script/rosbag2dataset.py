...
if __name__ == '__main__':
    ...
    for bag in os.listdir(bag_path):
        ...
        for topic, msg, t in bag.read_messages(topics=['/gripper_state']): #读取gripper_state
        ...
        for i in range(len(state_list)-1):#数据集格式,i_roll1_roll2_gripper.jpg
            ...
            img = cv.resize(img, (tactile_list[i].width, tactile_list[i].height))
            tactile_img = bridge.imgmsg_to_cv2(tactile_list[i], 'passthrough')
            #将tactile_img的第三个通道设置为img
            tactile_img[:,:,2] = img
            label= str(i)+'_'+str(state_list[i+1][0]-state_list[i][0])+'_'+str(state_list[i+1][1]-state_list[i][1])+'_'+str(state_list[i+1][2]-state_list[i][2])+'.jpg'
