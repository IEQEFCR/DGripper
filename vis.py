...
class Visualizer:
    def __init__(self):
        ...
        self.points[:, 0] = np.ndarray.flatten(self.X) * self.pixel_per_mm-640*0.5*self.pixel_per_mm
        self.points[:, 1] = np.ndarray.flatten(self.Y) * self.pixel_per_mm-480*0.5*self.pixel_per_mm
        self.points[:, 2] = np.ndarray.flatten(Z)

        self.pcd = [open3d.geometry.PointCloud(), open3d.geometry.PointCloud(), open3d.geometry.PointCloud()]
        for i in self.pcd:
        ...
        coord= open3d.geometry.TriangleMesh.create_coordinate_frame(size=20, origin=[0, 0, 0])
    ...
    def update(self, points, gradients,yaw,roll):
        ...
        for i in self.pcd:
            ...
            i.translate([40, 0, 0])
            i.rotate(open3d.geometry.get_rotation_matrix_from_xyz([0, -roll, 0]), center=(0, 0, 0))
        
        self.pcd[1].rotate(open3d.geometry.get_rotation_matrix_from_xyz([0, 0, -yaw]), center=(0, 0, 0))
        self.pcd[2].rotate(open3d.geometry.get_rotation_matrix_from_xyz([0, 0, yaw]), center=(0, 0, 0)) 
